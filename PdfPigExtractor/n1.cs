...
namespace PdfTableExtractor;
	...
	public static List<TableRectangle> MergeRowsIntoTables(List<TableRectangle> rows, float rowTolerance = 5f) {
		...
		foreach (var row in sorted) {
			if (currentTable == null) {
				...
				// Check vertical adjacency (Y coordinate difference)
				float verticalGap = currentTable.Y - (row.Y + row.Height);
				if (Math.Abs(verticalGap) <= rowTolerance) {
					// Expand table rectangle to include this row
